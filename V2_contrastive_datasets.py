...
from utils.directory import createDir
from utils.loading import LoadConfig
from utils.paths import CONFIG

from models.contrastive_models import class_contrastive_model
...
def accuracy_at_1(train_set, val_set, model, accuracy_calculator):
    ...
    with torch.no_grad():
        ...
        accuracies = accuracy_calculator.get_accuracy(
            query=train_embeddings,
            reference=val_embeddings,
...
def contrastive_features(loader: list, model, config, path_dir):
    ...
    dgl.save_graphs(
        str(config["root_dir"] / "graphs_contrastive" / path_dir), graphs
    )


"""
RECORDA QUE HAS CAMBIAT UNA PART DEL CODI FONT DE LA LLIBRERIA DELS TRIPLETS A LA FUNCIO compute_all_embeddings # noqa
xmin, ymin, xmax, ymax = box
"""
if __name__ == "__main__":

    parser = argparse.ArgumentParser(description="Training")
    parser.add_argument("--run-name", type=str, default="base")
    parser.add_argument("--checkpoint", type=str, default=None)
    args = parser.parse_args()

    config = LoadConfig(dir_name=CONFIG, args_name=args.run_name)
    ...
    print(
        "Distance metric {}".format(
            config["contrastive_learning"]["distance_metric"]
        )
    )

    distance = getattr(
        distances, config["contrastive_learning"]["distance_metric"]
    )()
    ...
    if config["pickle_path_train_kmeans"].endswith(".bin"):
        ...
        with open(
            config["pickle_path_train_kmeans"], "rb"
        ) as kmeans_train, open(
            config["pickle_path_val_kmeans"], "rb"
        ) as kmeans_val, open(
            config["pickle_path_test_kmeans"], "rb"
        ) as kmeans_test:
            train_graphs = pickle.load(kmeans_train)
            val_graphs = pickle.load(kmeans_val)
            test_graphs = pickle.load(kmeans_test)

    vector_func_partial = partial(
        vector_func, config
    )  # Partial function to use with apply_edges
    ...
    model = class_contrastive_model(**config).to(device)
    ...
    if config["pretrainedWeights"] is None:
        for epoch in range(config["epochs"]):
            loss = train(
                model, loss_func, mining_func, train_loader, optimizer, epoch
            )

            precision = accuracy_at_1(
                train_dataset, val_dataset, model, accuracy_calculator
            )
        ...
        # model = torch.load(os.path.join(config["weights_dir"], os.listdir(config["weights_dir"])[-1])) # noqa
        print("Skipping training")
        # Compute the test accuracy
    print("!!! Computing test accuracy !!!")
    precision = accuracy_at_1(
        train_dataset, test_dataset, model, accuracy_calculator
    )
